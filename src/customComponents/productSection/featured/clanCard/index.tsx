import {
  alpha,
  Avatar,
  Box,
  Card,
  CardActions,
  CardContent,
  CardMedia,
  CardProps,
  Divider,
  Stack,
  SxProps,
  Theme,
  Typography,
} from '@mui/material';
import Image, { StaticImageData } from 'next/image';
import { MLinkProps } from '@/src/customComponents/mLink';
import { isArray, map } from 'lodash';
import { cardAnimationSx } from '@cc/productSection/featured/common_cardSx';
import {
  ClanActivity,
  ClanLanguage,
  ClanStatus,
  ClanTag,
  ClanTimeZone,
  Count,
} from '@/src/commomTypes';
import { CampaignRounded, MonetizationOn } from '@nm/@mui/icons-material';
import StepIcon from '@cc/icons/stepIcon';
import SlotIcon from '@cc/icons/slotIcon';
import { getCount } from '@/src/utils';
import RegistrationIcon from '@cc/productSection/featured/clanCard/icon/registrationIcon';
import MatchIcon from '@cc/productSection/featured/clanCard/icon/matchIcon';

export interface ClanCardProps extends Omit<CardProps, 'children'> {
  data: {
    banner: string | StaticImageData;
    id: string | number;
    title: Capitalize<string>;
    tag: Capitalize<ClanTag>;
    status: Capitalize<ClanStatus>;
    activity: Capitalize<ClanActivity>;
    languages: Capitalize<ClanLanguage>[];
    clanTimeZone: Capitalize<ClanTimeZone>;
    rating: Count;
    rewards: Count;
    steps: Count;
    slots: Count;
    href: MLinkProps['href'];
  };
}

const ClanCard = (props: ClanCardProps) => {
  const { data, ...cardProps } = props;
  const { sx, ...restCardProps } = cardProps;
  const { banner, title, rewards, steps, slots, activity, tag } = data;

  const rewardStepsSlots = [
    {
      icon: MonetizationOn,
      label: `Reward USD ${getCount(rewards)}$`,
    },
    {
      icon: StepIcon,
      label: `Steps ${getCount(steps)}`,
    },
    {
      icon: SlotIcon,
      label: `Slots ${getCount(slots)}`,
    },
  ];

  return (
    <Card
      sx={[featureCardSx, cardAnimationSx, ...(isArray(sx) ? sx : [sx])]}
      {...restCardProps}
    >
      <Box position={'relative'} borderRadius={'inherit'} height={'390px'}>
        <CardMedia
          sx={{
            width: 1,
            height: 1,
            borderRadius: 'inherit',
            overflow: 'clip',
          }}
        >
          <Image
            src={banner}
            alt={title}
            width={305}
            height={410}
            style={{
              objectFit: 'cover',
            }}
            placeholder={'blur'}
          />
        </CardMedia>
        <Avatar sx={avatarBadgeSx}>
          <Image src={banner} alt={title} />
        </Avatar>
      </Box>
      <CardContent
        component={Stack}
        alignItems={'flex-start'}
        sx={{
          mt: 1,
          py: 1,
        }}
      >
        <Typography variant={'h6'} component={'span'} fontWeight={500}>
          #{title}
        </Typography>
        <Stack direction={'row'} width={1} justifyContent={'space-between'}>
          <Stack
            component={Stack}
            direction={'row'}
            columnGap={0.5}
            flexShrink={0}
            width={'fit-content'}
          >
            {activity === 'Announcement' ? (
              <CampaignRounded />
            ) : activity === 'Registration' ? (
              <RegistrationIcon />
            ) : (
              <MatchIcon />
            )}
            <Typography variant={'subtitle1'} component={'span'}>
              {activity}
            </Typography>
          </Stack>
          <Typography
            variant={'subtitle1'}
            component={'span'}
            sx={{
              color: (theme) => alpha(theme.palette.text.primary, 0.8),
            }}
          >
            {tag}
          </Typography>
        </Stack>
      </CardContent>
      <Divider
        flexItem
        variant={'middle'}
        sx={{
          border: 'none',
          height: '1px',
          opacity: 0.3,
          backgroundImage: ({
            palette: {
              info: { main },
            },
          }) => `linear-gradient(116.85deg, #FC466B 0%, ${main} 100%)`,
        }}
      />
      <CardActions
        sx={{
          px: 2,
          py: 1,
          flexDirection: 'column',
          alignItems: 'center',
        }}
      >
        <Stack
          direction={'row'}
          width={1}
          columnGap={2.5}
          justifyContent={'space-between'}
        >
          {map(rewardStepsSlots, (item, index) => {
            const { label, icon: Icon } = item;
            return (
              <Stack key={index} direction={'row'} columnGap={0.5}>
                <Icon />
                <Typography
                  variant={'subtitle1'}
                  component={'h2'}
                  fontWeight={500}
                >
                  {label}
                </Typography>
              </Stack>
            );
          })}
        </Stack>
      </CardActions>
    </Card>
  );
};

export default ClanCard;

const featureCardSx: SxProps<Theme> = () => {
  return {
    width: 304,
    height: 544,
    border: '1px solid',
    borderColor: alpha('#3F5EFB', 0.2),
  };
};
const avatarBadgeSx: SxProps<Theme> = () => {
  return {
    position: 'absolute',
    bottom: -10,
    left: 0,
    width: 68,
    height: 68,
    border: '1px solid',
    borderColor: 'common.white',
    img: {
      width: 'inherit',
      height: 'inherit',
      transform: 'none !important',
    },
  };
};